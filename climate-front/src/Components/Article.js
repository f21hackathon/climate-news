...
export default function ImgMediaCard({ article }) {
	...
	var trimmed_article_title =
		raw_article_title.length > ARTICLE_TITLE_LENGTH
	...
	var trimmed_author =
		raw_author.length > ARTICLE_AUTHOR_LENGTH
		...
		<Card className="article">
			...
			<CardContent>
				<Typography
					...
					<a className="article-link" href={article.url} target="_blank">
						{trimmed_article_title} - {trimmed_author}
					</a>
					<CopyToClipboard text={article.url} onCopy={article.url}>
						<div className="tooltip">
							<LinkIcon
								sx={{
									":hover": { boxShadow: 6, color: "success.main" },
								}}></LinkIcon>
