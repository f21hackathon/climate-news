...
const CountryStatsList = ({ countryData }) => {
	...
	const co2PerCapita = data
		? `${data[len - 1]["co2_per_capita"]} tonnes per person`
		: null;
	...
	const ghgPerCapita = data
		? `${data[len - 1]["ghg_per_capita"]} tonnes per person`
		: null;
		...
		<Box sx={{ flexGrow: 0, maxWidth: 752 }}>
			...
			<List>
				<ListItem>
					...
					<ListItemText primary="ISO" secondary={isoCode} />
				</ListItem>
				<ListItem>
					<Tooltip title="Country population" placement="top-start">
						<ListItemAvatar>
							<Avatar>
								<EmojiPeopleIcon />
							</Avatar>
				...
				<ListItem>
					...
					<ListItemText primary="CO2 per Capita" secondary={co2PerCapita} />
				</ListItem>
				<ListItem>
					<Tooltip
